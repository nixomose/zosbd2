...
#define TXT_DEVICE_NAME "zosbd2ramdisk"
#define TXT_ZOSBD2_CONTROL_DEVICE_NAME "zosbd2ctl"
...
class libzosbd2
  ...
  public:
    ...
    int create_block_device(int fd, const string &device_name, u32 block_size, u64 number_of_kernel_blocks, u32 device_timeout_seconds, u32 &handle_id_out)
      {
        ...
        op_create.create_params.bio_segment_batch_queue_size = 0;
    ...
    int validate_read_request(u32 handle_id, zosbd2_operation &requestop)
      {
    ...
    int block_for_request(int fd, u32 handle_id, zosbd2_operation &op)
      {
    ...
    int respond_to_write_request(int fd, u32 handle_id, zosbd2_operation &requestop)
      {
        i32 error = 0;
    ...
    int run(void)
      {
        ...
        string full_device_name = "/dev/" + device_name;
        int fd;
        ...
        u32 alloc_size = sizeof(zosbd2_operation) + MAX_BIO_SEGMENTS_BUFFER_SIZE_PER_REQUEST;

        void *buffer = malloc(alloc_size);
        if (buffer == NULL)
          return Ret::r(-ENOMEM, "Unable to allocate memory for zosbd2_operation: %d", ENOMEM);
